...
static void debounce_key(void)
{
...
#endif
    {
        ...
        {
            /* Debounce OK */
            p_ksi = new_keyh.field.input;
            p_buf = bscan_matrix[new_keyh.field.output];

            /* make */
            if(IS_MASK_CLEAR(p_buf, BIT(p_ksi)))
            {
                if(accept_same_new_key(new_keyh.field.output,
                    new_keyh.field.input) > 0)
                {
                    change_valid();
                }
                else
...
static void change_valid(void)
{
    ...
    if ((ITempB04 & ITempB03) != 0)
    {
        ...
        /* set accept time for next new key */
        last_new_accept_time = 50;
        last_new_key_kso_index = new_keyh.field.output;
        last_new_key_ksi_index = new_keyh.field.input;
    }
...
void service_scan(void)
{
    ...
    {
        Hook_scan_activity();       //OEM_SCANTABS.C
	}
    ...
    */
    {
        ...
        {
            /* Debounce OK */
            p_ksi = new_extendkey.field.input;
            p_buf =
                bscan_matrix[new_extendkey.field.output + STANDARD_SCAN_LINES];

            /* make */
            if(IS_MASK_CLEAR(p_buf, BIT(p_ksi)))
            {
                if(accept_same_new_key(
                    new_extendkey.field.output + STANDARD_SCAN_LINES,
                    new_extendkey.field.input) > 0)
                {
                    CheckExtendKeyValid();
                }
                else
...
void CheckExtendKeyValid(void)
{
    ...
    if ((ITempB02 & ITempB01) != 0)
    {
        ...
        /* set accept time for next new key */
        last_new_accept_time = 50;
        last_new_key_kso_index = new_extendkey.field.output+
            STANDARD_SCAN_LINES;
        last_new_key_ksi_index = new_extendkey.field.input;
    }
...
void service_scan(void)
{
        ...
        if(ksitemp!=0x00)
        {
            ...
            for(ksi_index=0x00; ksi_index<8; ksi_index++)
            {
                if(IS_MASK_SET(ksitemp, BIT(ksi_index)))
                {
                    ...
                    /* Break */
                    if(IS_MASK_SET(bscan_matrix[kso_index], BIT(ksi_index)))
                    {
                        /* Key break; */
                        Send_Scan2(new_keyh, BREAK_EVENT);
                        CLEAR_MASK(bscan_matrix[kso_index], BIT(ksi_index));
                        CLEAR_MASK(ksitemp, BIT(ksi_index));
                    }
                }
            }
        }
    ...
    for(kso_index=0x00; kso_index<(16+ExtendScanPin); kso_index++)
    {
        ...
        if(ksitemp!=0x00)
        {
            ...
            /* Check KSI0 ~ 7 for make */
            for(ksi_index=0x00; ksi_index<8; ksi_index++)
            {
                if(accept_same_new_key(kso_index, ksi_index) > 0)
                {
                if(IS_MASK_SET(ksitemp, BIT(ksi_index)))
                {
                    /* KSO16, KSO17 */
                    ...
                    if(IS_MASK_CLEAR(bscan_matrix[kso_index], BIT(ksi_index)))
                    {
                        ...
                        if(find_paths(ksitemp, kso_index)==FALSE)
                        {
                            ...
                            /* set accept time for next new key */
                            last_new_accept_time = 40;
                            last_new_key_kso_index = kso_index;
                            last_new_key_ksi_index = ksi_index;
...
void Scan_Init(void)
{
    ...
    SDC3R = WKSOLDLY_29us+SDLYBR_01ms;
}
 ...
 * ****************************************************************************
 *
 * reset kso/ksi index and accept time of new key
 *
 * @return
 *
 * @parameter
...
void reset_new_key_index(void)
{
    ...
    last_new_accept_time = 0x00;
}

/**
 * ****************************************************************************
 *
 * To check same new key accept or not
 *
 * @return
 * 1, accept
 * 0, no accept
...
{
    if((last_new_key_kso_index != p_kso) || (last_new_key_ksi_index != p_ksi))
    {
        return 1;
    }
    else
