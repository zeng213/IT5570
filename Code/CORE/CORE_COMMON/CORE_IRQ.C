...
{
	CLEAR_MASK(IER1,Int_WKO25);         // Disable INT14 Pwr-switch Interrupt
	ISR1 = Int_WKO25;                   // Clear INT14 pending Status	
	CLEAR_MASK(WUENR2, BIT5);
	WUESR2 = BIT5; 
...
void IRQ_INT101_WKO96(void)
{
    ISR12 = Int_WKO96;
    WUESR10 = BIT0;
    CLEAR_MASK(IER12, Int_WKO96);

    //BRAM[0x31]++;     //CAN DEL AFTER DEBUG
...
// eSPI interrupt.
//----------------------------------------------------------------------------
void IRQ_INT153_eSPI_INT(void)
{
    ISR19 = Int_eSPI;
    eSPI_CNT_INT153++;
    CLEAR_MASK(IER19, Int_eSPI);
}

//----------------------------------------------------------------------------
// eSPI VW interrupt.
//----------------------------------------------------------------------------
void IRQ_INT154_eSPI_VWINT(void)
{
#if ENABLE_INT154_QUICK_CTRL
        ...
        if (VWIDX3 & F_IDX3_PLTRST_VALID)
        {
            ...
            else
            {
                ...
                if (eSPI_PLTRST_TAG == F_PLTRST_HI_LEVEL)
                {
                    ...
                    AMP_EN_LO();	// For AMP_EN workaround
                }
            }
        }
    }
...
//
//----------------------------------------------------------------------------
void IRQ_INT155_P8081_INT(void)
{
    ISR19 |= 0x08;
}
//----------------------------------------------------------------------------
...
void Isr_Tmr0(void) interrupt 1 using 2
{
    ...
    if(last_new_accept_time > 0)
    {
        if(--last_new_accept_time == 0x00)
        {
            last_new_key_kso_index = 0xFF;
            last_new_key_ksi_index = 0xFF;
        }
    }
}
...
{
#if 1	/* Speed-up interrupt procedure */
    ...
    IRQ_INT152_Null,                    //   INT152 PECI Interrupt
    IRQ_INT153_eSPI_INT,                //   INT153 eSPI Interrupt
    IRQ_INT154_eSPI_VWINT,              //   INT154 eSPI VW Interrupt
    IRQ_INT155_P8081_INT,               //   INT155 Port 80h/81h Interrupt
    IRQ_INT156_Null,                    //   INT156 --
    IRQ_INT157_Null,                    //   INT157 External Timer 3 Interrupt
    IRQ_INT158_Null,                    //   INT158 --
    IRQ_INT159_Null,                    //   INT159 PLL Frequency Change Event

    IRQ_INT160_Null,                    //   INT160 SMBus E Interrupt
    IRQ_INT161_Null,                    //   INT161 SMBus F Interrupt
    IRQ_INT162_Null,                    //   INT162 --
    IRQ_INT163_Null,                    //   INT163 --
    IRQ_INT164_Null,                    //   INT164 --
    IRQ_INT165_Null,                    //   INT165 --
    IRQ_INT166_Null,                    //   INT166 --
    IRQ_INT167_Null,                    //   INT167 --

    IRQ_INT168_Null,                    //   INT168 --
    IRQ_INT169_Null,                    //   INT169 --
    IRQ_INT170_Null,                    //   INT170 --
    IRQ_INT171_Null,                    //   INT171 --
    IRQ_INT172_Null,                    //   INT172 --
    IRQ_INT173_Null,                    //   INT173 --
    IRQ_INT174_Null,                    //   INT174 --
    IRQ_INT175_Null,                    //   INT175 External Timer 4 Interrupt
};
...
void Isr_Int1 (void) interrupt 2 using 2
{
    ...
    if (IVECT == (154+0x10))
    {
        IRQ_INT154_eSPI_VWINT();	/* First Piority */
    }
    else
    {
        (IRQ_Service[(IVECT)])();	/* Dispatch to service handler. */
    }
    F_PowerDownRejectTag = 1;
    //BRAM[0x30]++;     // CAN DELETE AFTER DEBUG
#else
...
#ifdef SUPPORT_HOOK_ISR_INT1
#if SUPPORT_HOOK_ISR_INT1
    Hook_Isr_Int1();
#endif
#endif
}
...
void EnableModuleInterrupt(void)
{
    ...
    VWCTRL0 |= F_VW_INT_ENABLE;     //REG@3290.7 F_VW_INT_ENABLE
    //IER19 |= (Int_eSPI);          //REG@1151.1 Int_eSPI
    IER19 |= (Int_eSPI_VW);         //REG@1151.2 Int_eSPIVW
#endif
