...
BYTE CheckFnKeyBundledFlag(BYTE p_table_entry, BYTE p_event)
{
    ...
    {
        l_buffer_index = ((p_table_entry-SSKEY2_SPE_CODE)/8);  // index of buffer "FnKeyBundledFlag"
        l_buffer_mask = ((p_table_entry-SSKEY2_SPE_CODE)%8);   // mask of buffer "FnKeyBundledFlag"
        ...
        #endif
        {
            if(IS_MASK_CLEAR(FnKeyBundledFlag[l_buffer_index], BIT(l_buffer_mask))) //with fn key but no bundled fn key flag.
            {
                l_Fn_bundled = 0x01;            // with fn key and no bundled fn key flag.
            }
            else
            {
                l_Fn_bundled = 0x00;            // with fn key and with bundled fn key flag.
            }
        }
        else                                    // without Fn key
        {
            if(IS_MASK_CLEAR(FnKeyBundledFlag[l_buffer_index], BIT(l_buffer_mask)))
            {
                l_Fn_bundled = 0x00;            // no fn key and no bundled fn key flag.
            }
            else
            {
                l_Fn_bundled = 0x01;            // no fn key and with bundled fn key flag.
            }
        }
    }
    return(l_Fn_bundled);
